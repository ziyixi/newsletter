...
def rss(url="https://www.nature.com/articles/synthetic", published="2026-09-05"):
...
def test_crossref_metadata_never_asserts_abstract_or_follow_up_url_access():
    ...
    assert item["access_scope"] == "metadata" and item["provenance"] == "crossref_metadata"
...
@pytest.mark.parametrize(
    "raw", [b"[]", b"{}", b'{"message":null}', b'{"message":{"items":null}}', b"bad-json"]
)
...
def test_rss_rdf_titles_dates_and_links_are_metadata_only():
    ...
    assert item["published_at"] == "2026-09-05" and item["access_scope"] == "metadata"
...
def test_rss_future_date_is_not_current_evidence_and_rfc2822_is_supported():
    ...
    assert (
        sources.parse_rss(rss(published="Sat, 05 Sep 2026 16:00:00 GMT"), DAY)[0]["published_at"]
        == "2026-09-05"
    )
...
async def test_metadata_adapter_uses_only_fixed_https_origins_and_serial_bounded_public_queries(
    ...
    result = await sources.PublicMetadataFeed(transport=httpx.MockTransport(handler)).fetch(DAY)
...
async def test_one_feed_failure_degrades_without_discarding_other_sources(fast_metadata):
    def handler(request):
        if request.url.host == "api.crossref.org":
    ...
    result = await sources.PublicMetadataFeed(transport=httpx.MockTransport(handler)).fetch(DAY)
    assert len(result.candidates) == 1 and result.candidates[0]["provenance"] == "rss_metadata"
    assert "private" not in repr(result)


async def test_stream_limits_apply_to_decoded_response(fast_metadata, monkeypatch):
    ...
    result = await sources.PublicMetadataFeed(transport=httpx.MockTransport(handler)).fetch(DAY)
    assert not result.candidates and all("unavailable" in note for note in result.diagnostics)


async def test_timeout_degrades_and_cancellation_is_not_swallowed(fast_metadata):
    async def timeout(request):
        raise TimeoutError("private")

    assert not (
        await sources.PublicMetadataFeed(transport=httpx.MockTransport(timeout)).fetch(DAY)
    ).candidates
    ...
    with pytest.raises(asyncio.CancelledError):
        await sources.PublicMetadataFeed(transport=httpx.MockTransport(cancel)).fetch(DAY)
