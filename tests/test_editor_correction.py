...
async def test_url_and_approval_share_one_correction_budget(
    ...
    corrected = (
        FakeTurn(bundle) if corrected_search else limited_turn(bundle, "search")
    )
    ...
    assert (
        editor._result(text, [], opened, searched).review["passed"]
        is corrected_search
    )
...
def test_action_inspection_requires_an_exact_true_claim(value):
    assert (
        editor._unobserved_approval_actions(json.dumps(value), set(), False)
        == []
    )
