    ...
    value["withdrawals"] = [
        {
            ...
            "affected_signal_hash": content_hash(prior["signal"])
            if signal
            else "",
...
def delivery_receipt(
    repository, run_id, built, state="simulated", *, verification=False
):
...
def test_deadline_disclosure_does_not_call_approved_deep_a_brief_or_complete_issue():
    built = assemble(
        "run-1", DAY, [task()], [result(mode="deep")], reason="deadline"
    )
    assert built["coverage"]["mode"] == "partial"
...
def test_only_two_priority_deeps_and_other_complete_briefs_are_preserved():
    ...
    values = [
        result(i, mode) for i in range(1, 5) for mode in ("brief", "deep")
    ]
...
def test_later_failure_or_corrupt_result_does_not_replace_earlier_approved_checkpoint():
    ...
    corrupted = result(
        content=story(title="Unreviewed new text"), assessments=[]
    )
...
def test_explicit_independent_exact_body_withdrawal_keeps_unaffected_confirmed_signal():
    ...
    built = assemble(
        "run-1", DAY, [task(), task(2)], [prior, correction, result(2)]
    )
    assert [s["disposition"] for s in built["coverage"]["stories"]] == [
        "watch",
        "brief",
...
def test_generic_deep_failure_or_unverified_withdrawal_cannot_retract_good_brief(
    change,
):
    prior = result(signal=True)
    correction = withdrawal_result(prior)
...
def test_capacity_marks_whole_topic_deferred_instead_of_cutting_approved_paragraphs():
    ...
    assert [s["disposition"] for s in coverage[:MAX_SECTIONS]] == [
        "brief"
    ] * MAX_SECTIONS
    assert [s["disposition"] for s in coverage[MAX_SECTIONS:]] == [
        "deferred"
...
def test_twelve_briefs_keep_individual_titles_and_limitations_not_one_giant_world_block():
    values = [
        result(
            i,
            content=story(i, kind=SECTION_KINDS[(i - 1) % len(SECTION_KINDS)]),
        )
        for i in range(1, 13)
    ]
...
def test_immutable_checkpoint_replay_returns_detached_data_and_preserves_all_versions(
    repository,
):
    t = task()
    ...
    with pytest.raises(StoreError):
        repository.save(
            "run-1", t, "deep", result(mode="deep"), issue_date=NEXT_DAY
        )
...
def test_checkpoint_versions_and_per_run_story_count_are_bounded(repository):
    for i in range(MAX_VERSIONS):
        repository.save(
            ...
            task(),
            "brief",
            result(reason=f"checkpoint-{i}"),
            issue_date=DAY,
        )
    with pytest.raises(PublicationError) as caught:
        repository.save(
            "run-1", task(), "brief", result(reason="overflow"), issue_date=DAY
        )
...
def test_best_result_for_deep_prior_never_returns_last_failed_over_approved_brief(
    repository,
):
    initial = result(content=False, signal=True)
    brief = result()
...
def test_frozen_publication_is_reconstructed_from_ledger_and_not_mutated_by_late_deep(
    repository,
):
    ...
    built = assemble(
        "run-1", DAY, tasks, repository.results("run-1"), reason="deadline"
    )
    ...
    with pytest.raises(StoreError):
        repository.save(
            "run-1", tasks[0], "deep", result(mode="deep"), issue_date=DAY
        )
    with pytest.raises(StoreError):
...
def test_existing_combined_layout_replays_exactly_after_per_story_layout_upgrade(
    ...
    assert (
        freeze_publication(repository, "old-run", DAY, reason="restart") == old
    )
...
def test_freeze_reconstruction_preserves_admission_order_at_feature_and_paragraph_capacity(
    ...
    for selected in tasks:
        ...
        repository.save(
            "run-1", selected, "brief", result(number), issue_date=DAY
        )
        if number in (1, 2, 18):
            repository.save(
                "run-1",
...
def test_brief_watch_and_deferred_priorities_survive_next_day_history(
    ...
    values = [
        result(),
        result(2, content=False, signal=True),
        result(3, "deep"),
    ]
    repository.save_plan("run-1", DAY, tasks)
    for value in values:
