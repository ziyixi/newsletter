...
from test_publication_delivery import queue
...
from newsletter.email_templates import (
    MAX_TEMPLATE_BYTES,
    ...
    _fixture_context,
    render_template,
    template_from_inputs,
)
from newsletter.rendering import render_edition, validate_template
...
def test_template_cache_keys_the_immutable_source_not_one_global_template(source):
    ...
    assert render_template(first, _fixture_context(full=True)) == first_html
    assert _compiled.cache_info().currsize == 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "{% set bomb = 'many' %}{% for letter in bomb %}{{ letter }}{% endfor %}",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "<a href='https://example.org/' ping='https://example.org/track'>example</a>",
        ...
        "<p style='background:u/**/rl(https://example.org/tracker)'>example</p>",
        "<p style='background:image-set(\"https://example.org/tracker\")'>example</p>",
...
def test_validation_catches_accidentally_dropped_content(source, expression):
    ...
    with pytest.raises(TemplateValidationError, match="omits required"):
...
def test_bounded_iterations_include_data_loops_even_without_output(source):
    ...
    context = _fixture_context(full=False)
    ...
    fragment = "{% for a in references %}{% for b in references %}{% for c in references %}{% endfor %}{% endfor %}{% endfor %}"
    source += fragment
    validate_template(source)
    context["references"] *= 20
    with pytest.raises(TemplateValidationError, match="iteration limit"):
        render_template(source, context)


def test_source_and_output_sizes_are_bounded(source):
    with pytest.raises(TemplateValidationError, match="source is too large"):
        validate_template(source + " " * MAX_TEMPLATE_BYTES)
    context = _fixture_context(full=False)
    context["draft"]["introduction"] = "x" * MAX_TEMPLATE_OUTPUT_BYTES
    with pytest.raises(TemplateValidationError, match="output is too large"):
        render_template(source, context)
...
@pytest.mark.parametrize(
    "link",
    [
        "https://example.org/collect?q={{ personal.summary if personal else 'none' }}",
        "{{ references[0].url }}?private={{ personal.summary if personal else 'none' }}",
...
def test_runtime_link_allowlist_also_guards_unexercised_date_branch(source):
    changed = (
        source
        + """{% if issue_date == '2026-09-08' %}
        <a href="https://example.org/collect?q={{ personal.summary }}">extra link</a>
        {% endif %}"""
    )
...
def test_literal_budget_stops_expansion_before_single_macro_buffer_returns(source, monkeypatch):
    ...
    monkeypatch.setattr(Macro, "_invoke", record)
    context = _fixture_context(full=False)
    context["references"] *= 4
    with pytest.raises(TemplateValidationError, match="expansion budget"):
        render_template(changed, context)
