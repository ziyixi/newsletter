...
def editorial_policy():
    return files("newsletter").joinpath("policy/editorial.md").read_text(encoding="utf-8")
...
@pytest.mark.parametrize(
    "source", ["NeurIPS", "ICML", "ICLR", "ACL", "CVPR", "Nature", "Science", "arXiv"]
)
def test_research_discovery_accepts_multiple_venues_without_venue_as_proof(
    ...
    assert "声誉不等于证据" in editorial_policy and "声誉不等于证据" in reader_profile
...
def test_every_research_and_card_must_explain_the_evidence_without_a_click(editorial_policy):
    ...
    assert "只能读到摘要" in editorial_policy and "不编造全文细节" in editorial_policy
    assert "可省略卡片" in editorial_policy


def test_incompatible_population_counts_and_model_thresholds_are_not_conflated(editorial_policy):
    assert "WHO、OCHA" in editorial_policy
    assert "原文未明确的子集、包含、互斥或去重关系不得推断" in editorial_policy
    assert "不同日期、区域" in editorial_policy and "不可混用分母" in editorial_policy
...
def test_editor_recomputes_research_recency_instead_of_trusting_collection_note(editorial_policy):
...
def test_editor_recomputes_numeric_claims_and_preserves_cost_assumptions(editorial_policy):
...
def test_research_directions_require_honest_reading_scope_and_accessible_overview(direction):
    instruction = (
        files("newsletter").joinpath(f"instructions/{direction}").read_text(encoding="utf-8")
    )
