...
2. **真实连接预检**：启动必须通过专用 Codex 账号/模型目录、Notion 数据源 schema 和 Todofy public health 检查；不调用模型或发送邮件。公开 health 不证明 Todofy Basic Auth 有效，Notion 可读不证明可写，模型目录不证明搜索工具可用；后续真实请求另验。只输出通过/失败，不回显敏感响应。
