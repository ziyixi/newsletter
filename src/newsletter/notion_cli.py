"""Explicit schema setup and read-only sync diagnostics; never research or mail."""
...
def status(path: Path) -> dict[str, object]:
    with sqlite3.connect(path.resolve().as_uri() + "?mode=ro", uri=True) as db:
        ...
        if not db.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='notion_entities'"
        ).fetchone():
...
def main() -> None:
    ...
    try:
        if args.command == "setup":
            ...
            directory = Path(os.environ.get("NEWSLETTER_DATA_DIR", ".data"))
