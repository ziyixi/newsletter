...
class WorkflowError(RuntimeError):
    def __init__(self, code: str = "invalid_state") -> None:
        self.code = (
            code
            if code in {"not_found", "conflict", "invalid_state", "invalid_input"}
            else "invalid_state"
        )
...
class WorkflowRepository:
    ...
    def start(
        self, run_id: str, definition: WorkflowDefinition, inputs: dict[str, Any]
    ) -> dict[str, Any]:
        if (
        ...
        with self.store.transaction():
            ...
            if row:
                if row["definition_hash"] != definition_hash or row["inputs_hash"] != inputs_hash:
    ...
    def _write(self, run: dict[str, Any]) -> None:
        ...
        self.store.db.execute(
            "UPDATE workflow_runs SET body=? WHERE id=?", (canonical_json(run), run["id"])
        )

    def _artifact(self, run_id: str, node_id: str, item_id: str, value: Any) -> str:
        body, digest = json_value(value)
        identifier = content_hash({"run": run_id, "node": node_id, "item": item_id, "hash": digest})
    ...
    def artifacts(self, run_id: str, node_id: str | None = None) -> list[dict[str, Any]]:
        with self.store.lock:
            query, args = "SELECT * FROM workflow_artifacts WHERE run_id=?", [run_id]
            if node_id is not None:
                query += " AND node_id=?"
                args.append(node_id)
            rows = self.store.db.execute(query + " ORDER BY node_id,item_id", args).fetchall()
            return [
                {
                    "id": row["id"],
                    "node_id": row["node_id"],
        ...
        with self.store.transaction():
            ...
            if state["map_expanded"]:
                if state["map_hash"] != digest:
                    raise WorkflowError("conflict")
                return
            if run["state"] not in {"queued", "running"} or state["state"] != "pending":
                raise WorkflowError()
            if any(
                run["nodes"][dependency]["state"] not in SUCCESS_STATES for dependency in node.needs
            ):
                raise WorkflowError()
            state.update(
                map_expanded=True,
    ...
    def claim(self, run_id: str, node_id: str, item_id: str, input_value: Any) -> str | None:
        ...
        with self.store.transaction():
            ...
            if node is None:
                raise WorkflowError("invalid_input")
            definition = parse_definition(self.snapshot(run_id)["definition"])
            specification = next(item for item in definition.nodes if item.id == node_id)
            ...
            if any(
                ...
                return None
            target = (
                next((item for item in node["items"] if item["id"] == item_id), None)
                if item_id
                else node
            )
            if target is None or target["state"] != "pending":
                return None
            attempt_id = content_hash({"run": run_id, "node": node_id, "item": item_id})
            self.store.db.execute(
                "INSERT INTO workflow_attempts(id,run_id,node_id,item_id,state,input_hash,started_at) VALUES(?,?,?,?,?,?,?)",
                (attempt_id, run_id, node_id, item_id, "running", digest, now()),
            )
    ...
    def finish(self, attempt_id: str, state: str, value: Any = None, error_code: str = "") -> None:
        ...
        with self.store.transaction():
            ...
            definition = parse_definition(self.snapshot(run["id"])["definition"])
            specification = next(item for item in definition.nodes if item.id == attempt["node_id"])
            may_continue = (
                specification.on_error == "continue" and error_code not in FATAL_ERROR_CODES
            )
            target = (
                next(item for item in node["items"] if item["id"] == attempt["item_id"])
                if attempt["item_id"]
                else node
            )
            artifact = (
                self._artifact(run["id"], attempt["node_id"], attempt["item_id"], value)
                if state in SUCCESS_STATES
                else ""
            )
            target.update(state=state, artifact_id=artifact, error_code=error_code)
            self.store.db.execute(
            ...
            if attempt["item_id"]:
                if state in {"failed", "unknown"} and not may_continue:
                    ...
                    node.update(
                        ...
                        artifact_id=self._artifact(run["id"], attempt["node_id"], "", outputs),
                        degraded=any(
                            item["state"] in {"failed", "unknown"} for item in node["items"]
                        ),
                    )
                    if node["degraded"]:
                ...
                node.update(
                    ...
                    artifact_id=self._artifact(
                        run["id"], attempt["node_id"], "", [] if specification.map else None
                    ),
                )
            self._write(run)
    ...
    def recover(self) -> int:
        ...
        with self.store.transaction():
            ...
            for row in rows:
                ...
                if row["item_id"]:
                    item = next(item for item in node["items"] if item["id"] == row["item_id"])
