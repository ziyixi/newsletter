"""Small public metadata supplements, never a substitute for opening a primary source.
...
Public list requests: at most one/second, serial, no retries (2026-07-21 update):
...
def identity_keys(value: Mapping[str, object]) -> set[str]:
    """Comparison keys only. Never rewrite a source URL used for provenance/citation."""
    keys = set()
...
def deduplicate_candidates(
    ...
    *,
    ...
    """DOI/arXiv versions, URL aliases and shared event keys suppress repeat coverage.

    A dated/new-version follow-up is retained only with an explicit novelty reason;
    a watchlist is selection context, not permission to repeat an unchanged item.
    """
    if not 1 <= limit <= 60:
...
def _metadata_candidate(
    ...
    candidate: Candidate = {
        ...
        "why_now": "出版日期来自供应商元数据，进入选题前须核对版本、日期与实际新增发现。",
        ...
        try:
            ...
            for key in ("published-online", "published-print", "published"):
                ...
                if len(parts) == 3 and all(type(n) is int for n in parts):
                    published = date(*parts).isoformat()
                    break
...
class PublicMetadataFeed:
    """Only three fixed public HTTPS paths; never fetch a returned or operator URL.

    No credentials, proxies, redirects, retries, pagination, abstracts or full text.
